# Referências (não aquelas com &)


### Arrays
- [Arrays - C++ Tutorials (cplusplus.com)](https://www.cplusplus.com/doc/tutorial/arrays/)
- [Arrays in C/C++ - GeeksforGeeks](https://www.geeksforgeeks.org/arrays-in-c-cpp/)
- [C++ Arrays (With Examples) (programiz.com)](https://www.programiz.com/cpp-programming/arrays)
- [C++ Arrays (w3schools.com)](https://www.w3schools.com/cpp/cpp_arrays.asp)
- [C++ Programming Tutorial 45 - Intro to Arrays and Vectors - YouTube](https://www.youtube.com/watch?v=6WGK0Tfl9oU)

### Vectors
- [vector - C++ Reference (cplusplus.com)](https://www.cplusplus.com/reference/vector/vector/)
- [C++ Vectors (With Examples) (programiz.com)](https://www.programiz.com/cpp-programming/vectors)
- [c++ - What does std::vector look like in memory? - Stack Overflow](https://stackoverflow.com/questions/52330010/what-does-stdvector-look-like-in-memory)
- [c++ - Iterator Loop vs index loop - Stack Overflow](https://stackoverflow.com/questions/14373934/iterator-loop-vs-index-loop)
- [c++ - What are the complexity guarantees of the standard containers? - Stack Overflow](https://stackoverflow.com/questions/181693/what-are-the-complexity-guarantees-of-the-standard-containers)

### Tempo processamento
- [Measure execution time of a function in C++ - GeeksforGeeks](https://www.geeksforgeeks.org/measure-execution-time-function-cpp/)

### Ponteiros

- [Pointers - C++ Tutorials (cplusplus.com)](https://www.cplusplus.com/doc/tutorial/pointers/)
- [C++ Pointers (w3schools.com)](https://www.w3schools.com/cpp/cpp_pointers.asp)
- [c++ - When to use references vs. pointers - Stack Overflow](https://stackoverflow.com/questions/7058339/when-to-use-references-vs-pointers)
- [YouTube](https://www.youtube.com/watch?v=rtgwvkaYt1A)

### Declarações e definições
- [Declarations, Prototypes, Definitions, and Implementations](http://www.cplusplus.com/articles/yAqpX9L8/)

### Headers
- [Header - Microsoft Docs](https://docs.microsoft.com/pt-br/cpp/cpp/header-files-cpp?view=msvc-170)
- [.h or .hpp in class definition](https://stackoverflow.com/questions/152555/h-or-hpp-for-your-class-definitions)

### Queue
- [Queue with linked list](https://www.geeksforgeeks.org/queue-linked-list-implementation/)

### LinkedList
- [Linked List](https://www.tutorialspoint.com/data_structures_algorithms/linked_list_algorithms.htm)

### BinaryTree
- [Trees](https://www.tutorialspoint.com/data_structures_algorithms/tree_data_structure.htm)
- [BinaryTree](https://www.programiz.com/dsa/binary-search-tree)
- [BinaryTree print function](https://stackoverflow.com/questions/36802354/print-binary-tree-in-a-pretty-way-using-c)

